...
local function onActivate(e)
	...
	if #ref.object.inventory == 0 then
		...
		tes3.playSound{reference=ref, sound="Item Ammo Down", volume=0.5, pitch=0.9}
		...
		for i, stack in pairs(ref.object.inventory) do
			if stack.object.canCarry ~= false then
				...
				tes3.transferItem{from=ref, to=tes3.player, item=stack.object, count=stack.count}
			end
		end
		tes3.playSound{reference=ref, sound="Item Ingredient Up", volume=0.5, pitch=1.5}
